...
def analyze(fName):
	...
	plt.figure()
	plt.xlabel("Mean Cluster")
	plt.ylabel("Standard Deviation")
	plt.title("Avg Cluster Position vs Standard Deviation")
	clstr = []
	stdev = []
	for c in countries:
		clstr.append(np.mean(country_clusters[c]))
		stdev.append(np.std(country_clusters[c]))
	...
	plt.plot(clstr, stdev, 'o')
	plt.savefig("kmeans/" + fName + "/stdev_vs_mean")
	'''
	G=nx.Graph()
	...
	for c in countries:
		nbrs = list(map(lambda x: x[0], similar_to_country[c].most_common(1)))
		for n in nbrs:
			mc[n] += 1
			G.add_edge(c, n)
	nx.draw_random(G)#, labels=labels)
	plt.show()
	print(mc.most_common(10))
	'''
#for f in folders:
#	analyze(f)
for f in folders:
